...
make_dir(){
    ...
    echo "! Virtual Host Name: ${1}"
    echo "! Virtual Host Root: conf/sites/${1}/"
...
clean(){
    check_input ${1}
    docker compose exec ${CONT_NAME} su -s /bin/bash lsadm -c "cd /usr/local/lsws/conf && rm -rf sites/${1} && rm -rf vhosts/${1}"
}
